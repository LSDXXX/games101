...
Vector3f Scene::castRay(const Ray &ray, int depth) const
{
    ...
    if(hit.happened&&isValidF(hit_point.x)&&isValidF(hit_point.y)&&isValidF(hit_point.z)) {
        ...
        if(dist_shadow < 0.01) {
            auto cos_theta = dotProduct(N, wi);
            auto cos_theta_ = -dotProduct(sample_light.normal, wi);
            // shadow 
        ...
        if(get_random_float() < RussianRoulette) {
            auto wi = hit.m->sample(wo, N);
            
            auto pdf = hit.m->pdf(wo, wi, N);
            if(dotProduct(N, wi) < 0) {
                std::cout << "zero obj is micro: " << (hit.m->m_type == MICROFACET) << " pdf: " << pdf << std::endl;
            } 
            // if(dotProduct(wi, N) > 0) {
